<?PHP
	...
	$conn = oci_connect("system", "MLoveF6910", "//localhost/XE");
	if (!$conn) {
		$m = oci_error();
		echo $m['message'], "\n";
		exit;
...
<?PHP
	if(isset($_POST['submit'])){
		...
		$query = "SELECT * FROM AALOGIN WHERE ID = '$id' and Password='$oldpassword'";
		$parseRequest = oci_parse($conn, $query);
		oci_execute($parseRequest);
		// Fetch each row in an associative array
		$row = oci_fetch_array($parseRequest, OCI_RETURN_NULLS+OCI_ASSOC);
		if($row){
			$query1 = "UPDATE AALOGIN SET Password = '$newpassword' where ID = '$id'";
